...
class Config:
    ...
    -------------------------------------------------------------------------
    share_weight: bool
        If true --> spectral block is used repeatedly
    -------------------------------------------------------------------------
    factor: int
        Scale for number of features each 
    -------------------------------------------------------------------------
    ff_weight_norm: bool
        Enables weight normalization for the FF linear layers
    -------------------------------------------------------------------------
    n_ff_layers: int
        Number of linear layers inside the FF network
    -------------------------------------------------------------------------
    layer_norm: bool
        Normalization across feature channels at each grid point after the FF
        networks final layer
    -------------------------------------------------------------------------
    """

    epochs: int = 40
    batch_size: int = 5
    learning_rate: float = 0.0005 # 0.05 gave NaN issues after epoch 23, try 0.0005
    ...
    def __str__(self):
        overview = f"++++++++++ F-IFNO ++++++++++\n" \
            ...
            f"Factor scale (width)={self.factor}\n" \
            f"FF weight norm={self.ff_weight_norm} | FF layers={self.n_ff_layers}\n" \
            f"++++++++++++++++++++++++++++++\n"
        return overview
...
######################################################################
########## Load datasets --> make dataloaders; train, validation and test ##########
######################################################################
...
########## Load only training set ###########
################################################################################
def mean_std_train(dir_path, data_years, var_in_set, var_out_set):
    """ INPUTS
    dir_path : str
        Path of folder/directory where the datasets are contained
    ------------------------------------------------------------------------
