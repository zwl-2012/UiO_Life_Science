...
from config_IUFNO import Config, load_dataset_train, load_dataset, original_data_load


"""A collection of functions for IUFNO"""
...
class FNO2d(nn.Module):
    def __init__(self, setup):
        ...
        self.convlayer = nn.ModuleList([SpectralConv2d(self.width, self.width, self.modes1, self.modes1).to(device) for i in range(1)])
        ...
        self.enc = nn.Conv2d(self.var_in*self.T_in, self.width, 1)
        self.dec = nn.Conv2d(self.width, self.var_out*self.T_out, 1)
...
def tune_model(setup, train_years, val_years, folder_path, save_folder_weight, folder_save_MSE, 
    ...
    for ep in range(epochs_tuning):
        ...
        if val_loss_sum < curr_val_loss_sum: 
            ...
            this_path = f'{save_folder_weight}IUFNO_t2m_{ep+1}.pth'
